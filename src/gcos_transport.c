 ...
 * @brief Unified transport layer with protocol separation (based on v2)
 * 
 ...
 * 
 * Architecture: HAL → Transport → APDU Processing
 */
...
/* ============================================================================
 * Transport API Implementation (Unified)
 * ============================================================================ */
...
s8 gcos_transport_send_byte(u8 byte) {
    ...
    /* Send single byte via HAL */
    s16 sent = hal_write(&byte, 1);
    return (sent == 1) ? 0 : -1;
}

s8 gcos_transport_receive_byte(u8 *byte) {
    if (!transport_initialized || byte == NULL) {
        return -1;
    }
    
    /* Receive single byte via HAL */
